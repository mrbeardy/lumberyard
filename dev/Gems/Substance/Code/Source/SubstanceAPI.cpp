...
//this gets redefined in ImageExtensionHelper below
...
extern void* gSubstanceEngineModule;

//--------------------------------------------------------------------------------------------
class CSubstanceParameterXML
    : public ISubstanceParameterXML
{
public:
...
//----------------------------------------------------------------------------------------------------
class CSubstanceMaterialXMLData
    : public ISubstanceMaterialXMLData
{
public:
...
//--------------------------------------------------------------------------------------------
class CFileContents
    : public IFileContents
{
public:
    CFileContents(byte* data, size_t bytes)
        : m_Data(data)
        , m_Bytes(bytes)
    {
    }
...
void* CSubstanceAPI::GetSubstanceLibrary()
{
    return gSubstanceEngineModule;
}
...
ISubstanceMaterialXMLData* CSubstanceAPI::LoadMaterialXML(const char* path, bool bParametersOnly)
{
    ...
    if (mtlNode)
    {
        ...
        for (int i = 0; i < mtlNode->getChildCount(); i++)
        {
            ...
            {
                unsigned int uid;
                SSubstanceOutputInfoXML outputInfo;

                child->getAttr("Enabled", outputInfo.bEnabled);
                child->getAttr("Compressed", outputInfo.bCompressed);

                //get override children
                for (int c = 0; c < child->getChildCount(); c++)
                {
                    XmlNodeRef componentChild = child->getChild(c);

                    if (!strcmp(componentChild->getTag(), "ComponentOverride"))
                    {
                        unsigned int index = 0;
                        unsigned int outputID = SubstanceAir::OutputFormat::UseDefault;

                        componentChild->getAttr("Index", index);

                        //set shuffle index here, so it defaults to whatever index we read in
                        unsigned int shuffleIndex = index;

                        componentChild->getAttr("ShuffleIndex", shuffleIndex);
                        componentChild->getAttr("OutputID", outputID);

                        if (index < SubstanceAir::OutputFormat::ComponentsCount)
                        {
                            SubstanceAir::OutputFormat::Component& component = outputInfo.OverrideFormat.perComponent[index];

                            component.shuffleIndex = shuffleIndex;
                            component.outputUid = outputID;
                        }
                    }
                }
...
bool CSubstanceAPI::ReadTexture(const char* path, SSubstanceLoadData& loadData)
{
    ...
    {
        ITexture* pTexture = gEnv->pRenderer->EF_LoadTexture(path, FT_IGNORE_PRECACHE | FT_USAGE_READBACK | FT_NOMIPS | FT_DONT_STREAM);
        ...
        {
        case eTF_BC1:
            loadData.m_Format = SubstanceTex_BC1;
            break;
        case eTF_BC2:
            loadData.m_Format = SubstanceTex_BC2;
            break;
        case eTF_BC3:
            loadData.m_Format = SubstanceTex_BC3;
            break;
        case eTF_BC4U:
            loadData.m_Format = SubstanceTex_BC4;
            break;
        case eTF_BC5S:
            loadData.m_Format = SubstanceTex_BC5;
            break;
        case eTF_L8:
            loadData.m_Format = SubstanceTex_L8;
            break;
        case eTF_PVRTC2:
            loadData.m_Format = SubstanceTex_PVRTC2;
            break;
        case eTF_PVRTC4:
            loadData.m_Format = SubstanceTex_PVRTC4;
            break;
        case eTF_R8G8B8A8:
            loadData.m_Format = SubstanceTex_R8G8B8A8;
            break;
        case eTF_R16G16B16A16:
            loadData.m_Format = SubstanceTex_R16G16B16A16;
            break;
        default:
#if AZ_TRAIT_SUBSTANCE_UNKNOWN_TEXTURE_FORMAT_NAME
            const char* texFormatName = "<UNKNOWN>";
#else
        ...
        if (pSubstanceLibAPI)
        {
            ...
            for (int i = 0; i < pTexture->GetNumMips(); i++)
            {
                ...
                pTexture->Readback(i, [&](void* data, uint32 rowPitch, uint32 slicePitch)
                    {
                        memcpy(writeBuffer, data, mipSize);
                        return true;
                    });
...
{
    return AZ::IO::FileIOBase::GetInstance()->Remove("@devassets@/" + string(path));
}
