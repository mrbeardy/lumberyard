    ...
    case eNotify_OnIdleUpdate:
    {
        ...
        if (m_RenderUID == INVALID_PROCEDURALMATERIALRENDERUID)
        {
            ...
            if (m_QueueRenderGraph)
            {
                ...
                EBUS_EVENT_RESULT(m_RenderUID, SubstanceRequestBus, RenderASync, true);
...
bool TryGenerateLumberyardSuffix(string& label, SubstanceAir::ChannelUse outputChannel, SubstanceAir::RenderResult* editorPreview)
{
    ...
    case SubstanceAir::Channel_Normal:
        if (editorPreview->getTexture().pixelFormat == Substance_PF_RGBA)
        {
            label = "ddna";
        }
        else
    ...
    if (m_CurrentMaterial)
    {
        ...
        for (int g = 0; g < m_CurrentMaterial->GetGraphInstanceCount(); g++)
        {
            ...
            for (int o = 0; o < pGraph->GetOutputCount(); o++)
            {
                ...
                SubstanceAir::RenderResult* pEditorPreview = pOutput->GetEditorPreview();
                ...
                string label = "";
                if (!TryGenerateLumberyardSuffix(label, pOutput->GetChannel(), pEditorPreview))
                {
                ...
                const SubstanceTexture& texture = pEditorPreview->getTexture();

                const int width = texture.level0Width;
                const int height = texture.level0Height;
                const int numChannels = (texture.pixelFormat & Substance_PF_L) ? 1 : 4;

                memset(tgaHeader, 0, sizeof(tgaHeader));
                tgaHeader[2] = numChannels == 1 ? kTGA_L : kTGA_RGB;
                tgaHeader[12] = width & 0x00FF;
                tgaHeader[13] = (width & 0xFF00) / 256;
                tgaHeader[14] = height & 0x0FF;
                tgaHeader[15] = (height & 0xFF00) / 256;
                tgaHeader[16] = numChannels * 8;
                ...
                for (int y = height - 1; y >= 0; y--)
                {
                    ...
                    int offset = stride * y;
                    outFile.Write(static_cast<byte*>(texture.buffer) + offset, stride);
                }
...
void QProceduralMaterialEditorMainWindow::CreateMaterial(IProceduralMaterial* pProcMaterial)
{
    ...
    for (int g = 0; g < pProcMaterial->GetGraphInstanceCount(); g++)
    {
        ...
        if (CMaterial* pEditorMaterial = GetIEditor()->GetMaterialManager()->CreateMaterial(mtlPath.c_str()))
        {
            ...
            bool bHasGlossMap = false;

            struct GraphOutputTextureMap
            {
                SubstanceAir::ChannelUse srcType;
                EEfResTextures dstType;
            };

            GraphOutputTextureMap gotm[] =
            {
                { SubstanceAir::Channel_BaseColor, EFTT_DIFFUSE },
                { SubstanceAir::Channel_Diffuse, EFTT_DIFFUSE },
                { SubstanceAir::Channel_Normal, EFTT_NORMALS },
                { SubstanceAir::Channel_Specular, EFTT_SPECULAR },
                { SubstanceAir::Channel_Height, EFTT_HEIGHT },
            };
            ...
            for (auto o = 0; o < pGraph->GetOutputCount(); o++)
            {
                ...
                if (pOutput->GetChannel() == SubstanceAir::Channel_Glossiness)
                {
                    bHasGlossMap = true;
                }
                ...
                for (int gc = 0; gc < DIMOF(gotm); gc++)
                {
                    ...
                    {
                        SEfResTexture&  resTexture = shaderResources.m_TexturesResourcesMap[gotm[gc].dstType];    // by doing this we inserted new slot.
            ...
            shaderResources.m_LMaterial.m_Diffuse.Set(1.0f, 1.0f, 1.0f, 1.0f);

            //update specular and smoothness if glossiness map exists
            if (bHasGlossMap)
            {
                shaderResources.m_LMaterial.m_Specular.Set(1.0f, 1.0f, 1.0f, 1.0f);
                shaderResources.m_LMaterial.m_Smoothness = 255.0f;
            }
...
void QProceduralMaterialEditorMainWindow::CreateOutputPreviews(int graphIndex)
{
    ...
    if (m_CurrentMaterial && 0 <= graphIndex && graphIndex < m_CurrentMaterial->GetGraphInstanceCount())
    {
    ...
    else
    {
        ...
        if (widget == nullptr)
        {
            ...
            case GraphInputType::String:
                widget = new GIString(input, this);
                break;
            }
        }
    }
