...
void CopyPose(const vr::TrackedDevicePose_t& src, AZ::VR::PoseState& poseDest, AZ::VR::DynamicsState& dynamicDest, AZ::VR::HMDTrackingLevel trackingLevel)
{
    AZ::Matrix4x4 matrix;
    ...
    if (error == vr::EVRInitError::VRInitError_None)
    {
        ...
        else
        {
            ...
            if (error == vr::EVRInitError::VRInitError_None)
            {
                ...
                {
                    vr::HmdMatrix44_t projectionMatrix = m_system->GetProjectionMatrix(vr::EVREye::Eye_Left, 0.01f, 1000.0f, vr::EGraphicsAPIConvention::API_DirectX);
...
void OpenVRDevice::GetPerEyeCameraInfo(const EStereoEye eye, const float nearPlane, const float farPlane, AZ::VR::PerEyeCameraInfo& cameraInfo)
{
...
bool OpenVRDevice::CreateRenderTargets(void* renderDevice, const TextureDesc& desc, size_t eyeCount, AZ::VR::HMDRenderTarget* renderTargets[])
{
...
void OpenVRDevice::SetTrackingLevel(const AZ::VR::HMDTrackingLevel level)
{
    switch (level)
    {
        ...
        default:
            AZ_Assert(false, "Unknown tracking level %d requested for OpenVR device", static_cast<int>(level));
            break;
    }
...
const AZ::VR::Playspace* OpenVRDevice::GetPlayspace()
{
    vr::HmdQuad_t openVRSpace;
    bool valid = vr::VRChaperone()->GetPlayAreaRect(&openVRSpace);

    m_playspace.corners[0] = OpenVRVec3ToLYVec3(openVRSpace.vCorners[0]);
    m_playspace.corners[1] = OpenVRVec3ToLYVec3(openVRSpace.vCorners[1]);
    m_playspace.corners[2] = OpenVRVec3ToLYVec3(openVRSpace.vCorners[2]);
    m_playspace.corners[3] = OpenVRVec3ToLYVec3(openVRSpace.vCorners[3]);
    m_playspace.isValid = valid;

    return &m_playspace;
}

void OpenVRDevice::UpdateTrackingStates()
{
    /*
        WaitGetPoses is a very complicated method and the reason we re-did a lot of work on
        the VR tracking systems.
        PLEASE read the comments associated with WaitGetPoses in openvr.h before digging
        into any tracking related code with OpenVR. It's crucial to understanding the system.
        I also highly recommend watching the presentations from Alex Vlachos from Valve.

        WaitGetPoses is important because it:
        Properly predicts poses (for client applications like Lumberyard)
        Blocks until the compositor deems that we have a shot at hitting VSync

        We must call this from the render thread. Calling this from the main thread
        will result in rendering with incorrect poses.

        We cannot run this in a side-thread. We need to block the render thread to
        properly implement Valve's running start algorithm. There is no choice in the matter.

        Oculus does not have an equivalent call. The compositor blocks on ovr_SubmitFrame instead.

        TODO: also retrieve tracked poses for game simulation use
    */
    m_compositor->WaitGetPoses(m_trackedDevicePose, vr::k_unMaxTrackedDeviceCount, nullptr, 0);

    // Update tracking for all tracked devices
    for (int deviceIndex = 0; deviceIndex < vr::k_unMaxTrackedDeviceCount; ++deviceIndex)
    {
        AZ::VR::TrackingState::StatusFlags flags = 0;

        if (m_trackedDevicePose[vr::k_unTrackedDeviceIndex_Hmd].bPoseIsValid)
        {
            flags |= AZ::VR::HMDStatus_OrientationTracked | AZ::VR::HMDStatus_PositionConnected | AZ::VR::HMDStatus_CameraPoseTracked;
        }

        if (m_system->IsTrackedDeviceConnected(vr::k_unTrackedDeviceIndex_Hmd))
        {
        ...
        {
            const vr::ETrackedDeviceClass deviceClass = m_system->GetTrackedDeviceClass(deviceIndex);
            switch (deviceClass)
            {
            case vr::ETrackedDeviceClass::TrackedDeviceClass_HMD:
            {
                m_trackingState.statusFlags = flags;
                CopyPose(m_trackedDevicePose[deviceIndex], m_trackingState.pose, m_trackingState.dynamics, m_trackingLevel);
            }
            break;
            ...
            case vr::ETrackedDeviceClass::TrackedDeviceClass_Controller:
            {
                ...
                CopyPose(m_trackedDevicePose[deviceIndex], controllerTrackingState.pose, controllerTrackingState.dynamics, m_trackingLevel);
                controllerTrackingState.statusFlags = flags;
                m_controller->SetCurrentTrackingState(deviceIndex, controllerTrackingState);
            }
            break;
