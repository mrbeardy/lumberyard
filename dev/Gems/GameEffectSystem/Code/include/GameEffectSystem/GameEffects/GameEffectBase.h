...
protected:
    ...
    void SetMaterialOnEntity(_smart_ptr<IMaterial>  pMaterial, EntityId entityId,
    ...
    if ((pEntity) && (numParticles > 0) && (pParticleEmitter) && (maxHeightScale > 0.0f))
    {
        ...
        if (pCharacter)
        {
            ...
            if (pPose)
            {
                ...
                IActor* pActor = gEnv->pGame && gEnv->pGame->GetIGameFramework() ? gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(pEntity->GetId()) : nullptr;
...
//--------------------------------------------------------------------------------------------------
inline void CGameEffect::SetMaterialOnEntity(_smart_ptr<IMaterial>  pMaterial, EntityId entityId, PodArray<int>* pBodyAttachmentIndexArray)
{
...
// Desc: Sets material on entity
//--------------------------------------------------------------------------------------------------
inline void CGameEffect::SetMaterialOnEntity(_smart_ptr<IMaterial>  pMaterial, EntityId entityId, PodArray<string>* bodyAttachmentNameArray)
{
