...
CGameEffectsSystem::~CGameEffectsSystem()
{
    ...
    {
        IGameRules* pGameRules = GetISystem()->GetIGame()->GetIGameFramework()->GetIGameRulesSystem()->GetCurrentGameRules();
        if (pGameRules)
        {
...
{
    if (s_hasLoadedData == false && GetISystem()->GetIGame() && GetISystem()->GetIGame()->GetIGameFramework())
    {
        ...
        if (!rootNode)
        {
            CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING,
                "Could not load game effects data. Invalid XML file '%s'! ", GAME_FX_DATA_FILE);
            return;
        }

        IItemParamsNode* paramNode = GetISystem()->GetIGame()->GetIGameFramework()->GetIItemSystem()->CreateParams();
        if (paramNode)
        {
...
void CGameEffectsSystem::OnTick(float deltaTime, AZ::ScriptTimePoint time)
{
    Update(deltaTime);
}
...
//--------------------------------------------------------------------------------------------------
bool CGameEffectsSystem::OnInputChannelEventFiltered(const AzFramework::InputChannel& inputChannel)
{
#if DEBUG_GAME_FX_SYSTEM
    ...
    if ((GetISystem()->GetIConsole()->GetCVar("g_gameFXSystemDebug")->GetIVal()) && (debugEffectCount > 0))
    {
            ...
            inputChannel.IsStateBegan())
        {
            ...
            else if (inputChannelNameCrc32 == GAME_FX_INPUT_ResetParticleManager)
            {
                gEnv->pParticleManager->Reset(false);
            }
            else if (inputChannelNameCrc32 == GAME_FX_INPUT_PauseParticleManager)
            {
